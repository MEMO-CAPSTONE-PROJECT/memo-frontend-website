...
const PopUpEditAdminList: React.FC<PopUpEditAdminListProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // If password fields are not being edited, set password and confirmPassword to empty strings
    ...
    try {
      // Choose the appropriate validation schema based on the isEditingPassword state
  ...
  <MemoButton
    title={isEditingPassword ? "ยกเลิก" : "แก้ไข"} // Toggle button text
    variant="Bluebutton"
    onClick={() => setIsEditingPassword(!isEditingPassword)} // Toggle password editing
  />
</div>
