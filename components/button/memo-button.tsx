...
export default function MemoButton({ title, variant = "primary", size = "full" }: Readonly<MemoButtonProps>) {
    ...
    return (
      <button className={`font-medium text-title rounded-sm py-3 ${variants[variant]} ${sizes[size]}`}>
        {title}
      </button> 
    )
  }